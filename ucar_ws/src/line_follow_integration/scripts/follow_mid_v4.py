...
# -*- coding: utf-8 -*-
"""
follow_mid_v4 — 中道专用: 第一条白线旋转-70° → 拐角检测旋转 → 第二条白线停车

流程:
  对正 → 直行0.3s → PID等3s → 检测第一条白线 → 旋转-70°
  → 拐角检测(方向0) → 接近 → 旋转-70° → 直行0.5s → 摆正2s → PID巡航4s
  → 检测第二条白线 → 停车

用法: rosrun car_server follow_mid_v4.py
"""
...
class FollowMidV4:
    def __init__(self):
        rospy.init_node('follow_mid_v4', anonymous=True)
        ...
        self.cruise_start = None; self.cruise_dur = 4.0
        ...
        self.fc = 0
        rospy.loginfo("✅ follow_mid_v4 第一条白线→旋转-70°→拐角→旋转→第二条白线→停车")
    ...
    def detect_stop_line(self, bgr):
        """第二条白线: 前线 → 30帧 → 后线 → 停车, 超时重置"""
        # 超时重置(只一次): 前白线后3s内无后线 → 误判, 重新找
        if self.stop_front_found and self.stop_front_time is not None \
                and self.stop_reset_count < 1:
            if (rospy.Time.now()-self.stop_front_time).to_sec() > self.stop_reset_timeout:
                self.stop_front_found = False; self.stop_fps = 0
                self.stop_voice = False; self.stop_front_time = None
                self.stop_reset_count += 1
                rospy.loginfo("⏰ 前白线超时, 重置(仅一次)!")
                return False
        ...
        if self.detect_white_line(bgr):
            if not self.stop_front_found:
                ...
                self.stop_front_found=True; self.stop_front_time=rospy.Time.now()
                return False
            elif self.stop_fps>=self.stop_fps_thr:
                rospy.loginfo("🛑 第二条白线(后)! 停车!"); return True
        return False
        ...
        try:
            ...
            if self.post_done:
                ...
                if self.detect_stop_line(frame):
                    ...
                    return
                t = self._pid(err, True); t.linear.x = min(t.linear.x, 0.25)
