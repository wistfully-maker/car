...
# -*- coding: utf-8 -*-
"""
follow_right_v4 — 时间制接近拐角
拐角确认(L0%>40% AND R0%>40%连续3帧) → 低速前进APPROACH_TIME秒 → 旋转65° → 微调 → 摆正直行2s → 停车

用法: rosrun car_server follow_right_v4.py
"""
...
APPROACH_TIME = 0.8   # 拐角确认后前进秒数
APPROACH_SPEED = 0.15  # 接近速度

class FollowRightV4:
    def __init__(self):
        rospy.init_node('follow_right_v4', anonymous=True)
        ...
        self.fc = 0
        rospy.loginfo(f"✅ follow_right_v4 时间制接近={APPROACH_TIME}s → 旋转65°")
