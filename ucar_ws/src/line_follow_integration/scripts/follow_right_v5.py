...
# -*- coding: utf-8 -*-
"""
follow_right_v5 — 时间制接近拐角
拐角确认(L0%>40% AND R0%>40%连续3帧) → 低速前进APPROACH_TIME秒 → 旋转65° → 微调 → 摆正直行2s → 停车

用法: rosrun car_server follow_right_v5.py
"""

import rospy, cv2, math, time, numpy as np
...
class FollowRightV5:
    def __init__(self):
        rospy.init_node('follow_right_v5', anonymous=True)
        ...
        rospy.loginfo(f"✅ follow_right_v5 时间制接近={APPROACH_TIME}s → 旋转65°")
