...
// 导航相关全局变量（供 aiuiMain.cpp extern 引用）
char retrieve_item[64] = {0};
bool nav_ready = false;
bool audio_session_begin = false;
int audio_session_countdown = 0;

/**
 * @brief 从用户语音中解析"取得XX"，提取物品名并触发导航
 */
void parseNavCommand(const std::string& text)
{
    auto pos = text.find("取得");
    if (pos != std::string::npos)
    {
        std::string item = text.substr(pos + strlen("取得"));
        // 去除尾部标点/空格
        while (!item.empty() &&
               (item.back() == '。' || item.back() == '，' ||
                item.back() == '.'  || item.back() == ','  ||
        ...
        if (!item.empty())
        {
            strncpy(retrieve_item, item.c_str(), sizeof(retrieve_item) - 1);
            retrieve_item[sizeof(retrieve_item) - 1] = '\0';
            nav_ready = true;
            std::cout << ">>>>>解析到待取物品: " << retrieve_item << std::endl;
            gTTS("好的，前往物品领取区取" + item);
        }
    }
}
...
//#endif
				...
				if (resultStr.size() > 20)
				{
					...
					parseNavCommand(res_question_str);
...
void AIUITester::test()
{
	...
	*/
	...
	while(1)
	{
		...
		// 唤醒后重置音频会话计数
			if (audio_session_begin)
			{
				audio_session_begin = false;
				audio_session_countdown = 250; // ~8秒窗口
			}
			if (audio_session_countdown > 0) audio_session_countdown--;
