...
void test_callback()
{
    ...
    ROS_INFO("检测到语音唤醒，自动启动导航与脚本");
     // 可选：唤醒播放提示音（自带唤醒提示音频）
    char wav_buf[3000];
    strcpy(wav_buf, package_path);
    string wake_wav = string(wav_buf).append("/audio/wakeup.wav");
    string play_cmd = "aplay " + wake_wav + " &";
    system(play_cmd.c_str());
    // 1、后台启动导航launch，&代表后台运行不阻塞程序
