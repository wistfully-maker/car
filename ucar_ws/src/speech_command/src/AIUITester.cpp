...
static std::atomic<bool> aiui_stopped(false);
static CompetitionCommandGate competition_gate;
		...
		if (sub == "iat")
		{
			...
			if (!contentId.empty())
			{
				...
				if (NULL != buffer)
				{
					...
					cout << ">>>>>云端识别iat:" << resultStr << endl;
					Json::Reader iatReader;
					Json::Value iatJson;
					if (iatReader.parse(resultStr, iatJson))
					{
						Json::Value wsArray = iatJson["text"]["ws"];
						bool isLast = iatJson["text"]["ls"].asBool();
						string fullText = "";
						for (unsigned int i = 0; i < wsArray.size(); i++)
							fullText += wsArray[i]["cw"][0]["w"].asString();
						// 处理所有非空结果，不只依赖 isLast
						if (!fullText.empty())
						{
							cout << ">>>>>识别文字:" << fullText << " (last=" << isLast << ")" << endl;
							CommandDecision decision = competition_gate.accept(fullText);
							if (decision.kind == CommandKind::NOISE)
							{
								cout << ">>>>>忽略唤醒词尾音:" << decision.text << endl;
								return;
							}
							if (decision.kind == CommandKind::INCOMPLETE_TASK)
							{
								cout << ">>>>>比赛任务尚不完整，继续监听:" << decision.text << endl;
								return;
							}
							if (decision.kind == CommandKind::DUPLICATE_TASK)
							{
								cout << ">>>>>忽略本轮重复比赛任务" << endl;
								return;
							}
							if (decision.kind == CommandKind::COMPLETE_TASK)
							{
								strcpy(question, decision.text.c_str());
								sign_conversation_cloud = 1;
								cout << ">>>>>完整比赛任务:" << decision.text << endl;
								gStop();
								return;
							}
							cout << ">>>>>忽略非比赛语音:" << fullText << endl;
							return;
						}
					}
				}
...
void AIUITester::createAgent()
{
	...
	appidJson["key"] = key;
	appidJson["api_secret"] = api_secret;
...
void gStop()
{
	if (NULL != globalAgent)
	{
		...
		globalAgent->sendMessage(stopMsg);
		stopMsg->destroy();
		aiui_stopped.store(true);
	}
	cout << ">>>>>AIUI已停止录音" << endl;
}
...
{
	cout << ">>>>>TTS:" << text << endl;
	string cmd = "python3 " + string(package_path) + "/scripts/tts_http.py \"" + text + "\" > /tmp/tts_debug.log 2>&1";
	int ret = system(cmd.c_str());
	if (ret != 0)
	{
		cout << ">>>>>TTS失败(ret=" << ret << "),日志:" << endl;
		system("cat /tmp/tts_debug.log 2>/dev/null");
		string wav = string(package_path) + "/audio/offline.wav";
		string cmd2 = "aplay -q " + wav + " 2>/dev/null";
		system(cmd2.c_str());
	}
	gStop();
}
...
static int find_sync_head(const unsigned char *msg, unsigned int msglen)
{
    for (unsigned int i = 0; i + 1 < msglen; ++i)
    {
        if (msg[i] == SYNC_HEAD && msg[i + 1] == SYNC_HEAD_SECOND)
        {
...
void uart_rec(const unsigned char *msg, unsigned int msglen)
{
    ...
    if (big_buf == NULL && recv_index + msglen >= 2) 
    {
	    ...
	    {
                int sync_offset = find_sync_head(msg, msglen);
                if (sync_offset >= 0)
                {
                    cout << "recv data before SYNC HEAD, resync" << endl;
                    uart_rec(msg + sync_offset, msglen - sync_offset);
                }
                else if (msg[msglen - 1] == SYNC_HEAD)
                {
                    recv_buf[0] = SYNC_HEAD;
                    recv_index = 1;
                }
                else
                {
                    cout << "recv data without SYNC HEAD, drop" << endl;
                }
                return;
            }
        }
	else if (recv_index == 1)
	{
	    ...
	    {
                cout << "recv partial SYNC HEAD mismatch, resync" << endl;
                recv_index = 0;
                uart_rec(msg, msglen);
                return;
            }
