"""Pure path-geometry logic for omni corner speed limiting.

Port of ucar_nav/scripts/lateral_mode_logic.py（车端已验证的弯道模式
切换逻辑），参数集改为全向版的两档：STRAIGHT 用基准全向参数，
CORNER 用降速参数。本模块不导入任何 ROS 依赖。
"""
...
def corner_mode_parameters(mode, straight, corner):
    """Return the TEB parameters for STRAIGHT or CORNER mode."""
    if mode == "STRAIGHT":
        return dict(straight)
    if mode == "CORNER":
        return dict(corner)
    raise ValueError("unknown corner mode: {}".format(mode))


class OmniCornerStateMachine:
    """Enter CORNER when the upcoming path bends, exit after heading aligns."""
