#!/usr/bin/env python3
"""
ROS巡线 V13 — 中间路段(右转版): 先直行5s→右转45°→巡线→白线直行8s停车
管线: Canny→IPM(dilate15+erode7)→八邻域→中线/墙线→PID

逻辑:
  启动后: 直行5秒(0.25m/s)→停车→原地右转50°→开启视觉巡线
  检测到白线: 直行8秒→停车
"""
...
INIT_STRAIGHT_TIME = 4.6    # 初始直行时间(秒)
TURN_ANGLE_DEG = 45         # 初始右转角度
TURN_SPEED = 0.873          # 原地转向角速度=50°/1s


class LineFollowerV13Stop:
    def __init__(self):
        ...
        rospy.loginfo("✅ V13 先直行5s→右转50°→巡线→停车 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if self.phase < 3:
                ...
                if self.phase == 0:
                    ...
                    if elapsed < turn_dur:
                        ...
                        rospy.loginfo("↩ 右转完成, 直行过渡2s...")
                elif self.phase == 2:
                    if elapsed < 3.0:
                        t=Twist(); t.linear.x=0.1; self.pub_cmd.publish(t)
                    else:
            ...
            if Ls and Rs:
                ...
                info=(f"V13 F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V13 F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V13 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V13: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v13")
    LineFollowerV13Stop()
    rospy.spin()
