...
# === PID(对标UP主) ===
KP, KI, KD = 0.2, 0.0, 0.05  # Kp降低平滑转弯
...
class LineFollowerV16Stop:
    def __init__(self):
        ...
        rospy.loginfo("✅ V16 停车检测+直行5秒 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if Ls and Rs:
                ...
                info=(f"V16 F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V16 F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V16 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V16: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v16")
    LineFollowerV16Stop()
    rospy.spin()
