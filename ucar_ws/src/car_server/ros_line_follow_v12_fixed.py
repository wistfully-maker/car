#!/usr/bin/env python3
"""
ROS巡线 V12F — 中间路段: 先直行5s→左转50°→巡线→白线直行8s停车
...
TURN_ANGLE_DEG = 45         # 初始左转角度
TURN_SPEED = 0.873          # 原地转向角速度=50°/1s


class LineFollowerV12FStop:
    def __init__(self):
        ...
        rospy.loginfo("✅ V12F 先直行5s→左转50°→巡线→停车 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if Ls and Rs:
                ...
                info=(f"V12F F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V12F F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V12F F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V12F: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v12f")
    LineFollowerV12FStop()
    rospy.spin()
