...
视觉+PID+发指令 全部在 image_callback(和V7完全一致)
scan_callback 只更新 obstacle_detected
odom_callback  只更新 latest_pose

避障: 巡航阶段检测到前方挡板 → 四步机动(右移0.42→转14°→前进0.5→左移0.42) → 回巡航

用法: rosrun car_server follow_national_left.py
"""
...
AVOID_POINT_THR = 10
AVOID_CONSEC = 3
STRAFE_OUT_DIST = 0.42
FORWARD_DIST = 0.45
STRAFE_IN_DIST = 0.42
MANEUVER_ROTATE_DEG = -25  # 顺时针25°
STRAFE_SPEED = 0.2
FORWARD_SPEED = 0.25


class NationalLeftFollower:
    def __init__(self):
        rospy.init_node('follow_national_left', anonymous=True)
    ...
    def detect_stop_line(self, bgr):
        ...
        if len(rf)>=6 and sum(rf)/len(rf)>ft:
            rospy.loginfo("🛑 前白线!")
            self.rotate(-16, 0.3); rospy.sleep(0.3)
    ...
    def _run_maneuver(self):
        ...
        elif self.maneuver_step == 1:  # 顺时针14°
            rospy.loginfo("✅ 顺时针旋转14°")
        ...
        elif self.maneuver_step == 3:  # 左移0.42
        ...
        try:
            ...
            if self.post_done:
                ...
                if self.stop_front_found:
                    ...
                    if self.fc%30==0: rospy.loginfo(f"🔍 找停车线中... 速度={t.linear.x:.2f}")
                self.cmd_pub.publish(t)
                return
...
if __name__=='__main__':
    try: NationalLeftFollower(); rospy.spin()
