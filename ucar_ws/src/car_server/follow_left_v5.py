...
class FollowLeftV5:
    def __init__(self):
        rospy.init_node('follow_left_v5', anonymous=True)
        ...
        self.stop_timeout = 2.4           # 前线后行驶超此时长强制停车（秒）
        self.stop_timeout_enable = False  # 超时强制停车开关（先关闭，观察降速效果）

        self.fc = 0
        rospy.loginfo(f"✅ follow_left_v5 时间制接近={APPROACH_TIME}s → 旋转65°")
    ...
    def detect_stop_line(self, bgr):
        ...
        if self.stop_timeout_enable and self.stop_front_found and self.stop_front_time is not None:
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if self.post_done:
                ...
                if self.fc%30==0: rospy.loginfo(f"🔍 找停车线中... 速度={t.linear.x:.2f} m/s")
                return
