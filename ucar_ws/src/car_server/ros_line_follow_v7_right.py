#!/usr/bin/env python3
"""
ROS巡线 V7 — 右弯赛道专用: 强制左墙巡线 + 降低Kp防甩出
管线: Canny→IPM(dilate15+erode7)→八邻域→左墙+偏移→PID

和V6区别:
  - 不自动切换左右墙, 强制沿左墙(避免右线反光干扰)
  - Kp降到0.15, 右弯更平缓
"""
...
BASE_SPEED, MAX_ANGULAR = 0.07, 0.3
DEADZONE = 15
LOOKAHEAD = 10
FORCE_LEFT_WALL = True


class LineFollowerV7Right:
    def __init__(self):
        ...
        rospy.loginfo("✅ V7 右赛道专用(左墙+Kp0.15) 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if Ls and Rs:
                ...
                info=(f"V7-R F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V7-R F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V7-R F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")
...
if __name__=="__main__":
    rospy.init_node("line_follower_v7_right")
    LineFollowerV7Right()
    rospy.spin()
