#!/usr/bin/env python3
"""
ROS巡线 V6F-Fixed — V6F + 一条线两边缘误判检测
管线: Canny→IPM(dilate15+erode7)→八邻域→中线/墙线→PID

和V6F区别: 左右边界距离<40像素 → 视为同一线两边缘, 强制切单墙模式
"""
...
class LineFollowerV6Fixed:
    def __init__(self):
        ...
        rospy.loginfo("✅ V6F IPM+八邻域+墙切换 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if Ls and Rs:
                ...
                info=(f"V6F F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V6F F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V6F F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V6F: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v6f")
    LineFollowerV6Fixed()
    rospy.spin()
