...
# 停车检测
STOP_WIN_H = 10; STOP_WIN_W = 60; STOP_WHITE_THRESH = 0.35
STOP_CONSECUTIVE = 1; STOP_DELAY = 2.0  # 降至2帧, 更容易触发


class LineFollowerV20Right:
    def __init__(self):
        self.bridge = CvBridge()
        self.last_error = 0.0; self.integral = 0.0; self.road_half = 45
        self.fc = 0; self.mode = "INIT"
        self.phase = 0; self.phase_time = None
        self.vision_start = None  # 视觉巡线开始时间(开局降速用)
    ...
    def callback(self, msg):
        try:
            ...
            if self.phase < 3:
                ...
                if self.phase == 0:
                    ...
                    if el < INIT_STRAIGHT2:
                        t=Twist(); t.linear.x=INIT_SPEED; self.pub_cmd.publish(t)
                    else: self.phase=3; self.phase_time=None; self.last_error=0.0; self.integral=0.0
                    self.vision_start=rospy.Time.now(); rospy.loginfo("✅ 前置完成, phase=3 开始巡线!")
                return
            ...
            if Ls and Rs:
                ...
                if self.vision_start and (rospy.Time.now()-self.vision_start).to_sec()<3.0:
                    twist.linear.x=BASE_SPEED*0.6  # 开局3秒降速防反光
                else:
...
if __name__=="__main__":
    rospy.init_node("line_follower_v20_right")
    LineFollowerV20Right()
    rospy.spin()
