...
# -*- coding: utf-8 -*-
"""
follow_left_v6 — 基于 V5，停车机制换成 line_follower 版
拐角确认(L0%>40% AND R0%>40%连续3帧) → 低速前进APPROACH_TIME秒 → 旋转65° → 微调 → 摆正直行2s → 停车

相对 V5 的改动：
  1. 停车动作：写 /tmp/stop_done.txt → 换成 stop_robot()（发 /finish_follow + 播 finish.wav + 节点退出）
  2. 停车线检测逻辑（detect_stop_line）保持不变，前线微调仍固定 -16°

用法: rosrun car_server follow_left_v6.py
"""

import rospy, cv2, math, numpy as np, os, subprocess
...
class FollowLeftV6:
    def __init__(self):
        rospy.init_node('follow_left_v6', anonymous=True)
        ...
        self.fc = 0
        rospy.loginfo(f"✅ follow_left_v6 时间制接近={APPROACH_TIME}s → 旋转65°")
    ...
    def stop_robot(self):
        """停车：零速 + 发 /finish_follow=True + 播 finish.wav + 节点退出。"""
