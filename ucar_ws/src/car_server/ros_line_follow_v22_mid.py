#!/usr/bin/env python3
"""
ROS巡线 V22 — 双白线检测: 第1条白线左转45° + 第2条白线停车
策略:
  0. 直行2s(0.2m/s) → 视觉巡线
  1. 顶部+底部双窗口检测白线 → 第1条白线: 逆时针转45° → 强制RIGHT_WALL
  2. 第2条白线 → 直行4.5s → 停车
"""
...
INIT_STRAIGHT = 1.8; INIT_SPEED = 0.2
TURN_ANGLE = 45; TURN_SPEED = 0.785
STOP_THRESH = 0.40; STOP_CONS = 1; STOP_DELAY = 6.3


class LineFollowerV22Mid:
    def __init__(self):
        self.bridge = CvBridge()
        self.last_error = 0.0; self.integral = 0.0; self.road_half = 50
        self.fc = 0; self.mode = "INIT"
        self.init_done = False; self.init_start = None
        self.white_count = 0; self.white_consec = 0
        self.width_history = []      # 路宽历史
        self.fork_done = False       # 岔口扩展触发
        self.fork_time = None        # 扩展后直行计时
    ...
    def callback(self, msg):
        try:
            ...
            if self.white_count == 0:
                mid = binary[rh//3:2*rh//3, rw//4:3*rw//4]
                row_r = (mid>0).sum(axis=1)/mid.shape[1]
                wr = row_r.max()
                if wr >= STOP_THRESH:
                    self.white_consec += 1
                    if self.white_consec >= STOP_CONS:
                        rospy.loginfo(f"↪ 第1条白线! 左转{TURN_ANGLE}°")
                        td=math.radians(TURN_ANGLE)/TURN_SPEED; t0=rospy.Time.now()
                        while (rospy.Time.now()-t0).to_sec()<td:
                            t=Twist(); t.angular.z=TURN_SPEED; self.pub_cmd.publish(t); rospy.sleep(0.05)
                        self.white_count=1; self.white_consec=0
                        self.last_error=0.0; self.integral=0.0
                        rospy.loginfo("✅ 左转完成, 强制RIGHT_WALL!")
                else: self.white_consec = max(0,self.white_consec-1)

            # === 停车检测(第2条白线) ===
            if self.white_count == 1 and not self.stop_detected:
                half = binary[rh//2:rh, rw//4:3*rw//4]
                br = (half>0).sum(axis=1)/half.shape[1]
                best = br.max()
                if best >= STOP_THRESH:
                    self.stop_consec += 1
                    rospy.loginfo(f"🛑 第2条白线 {best*100:.0f}% [{self.stop_consec}/{STOP_CONS}]")
            ...
            if Ls and Rs:
                ...
                if (lb[la]!=-1).sum()>5 and (rb[la]!=-1).sum()>5:
                    ...
                    if len(vl)>3 and len(vr)>3:
                        ...
                        # 岔口扩展检测: 路宽突变>1.5倍
                        self.width_history.append(self.road_half)
                        if len(self.width_history)>15: self.width_history.pop(0)
                        if not self.fork_done and len(self.width_history)>=10:
                            old_w = np.mean(self.width_history[:5])
                            new_w = np.mean(self.width_history[-5:])
                            if new_w > old_w * 1.5 and new_w > 50:
                                self.fork_done=True; self.fork_time=rospy.Time.now()
                                rospy.loginfo(f"🔀 检测到岔口扩展! 路宽{old_w:.0f}→{new_w:.0f}, 直行1.5s...")

                # 岔口扩展后→直行(等待白线)
                if self.fork_done and self.white_count == 0:
                    error=0.0; self.mode="FORK_STRAIGHT"
                # 一拐后强制右墙
                elif self.white_count >= 1:
                    # 一拐后: 强制右墙, 停车时切CENTER
                    if self.stop_detected and nL_all>20 and nR_all>20:
                ...
                spd = 0.1 if self.white_count>=1 else BASE_SPEED
                ...
                info=f"V22 F{self.fc} {self.mode} e={error:.0f} W{self.white_count}"
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V22 F{self.fc} LOST",(5,12),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,0,255),1)

            self._dbg(dbg,msg)
            if self.fc%30==0: rospy.loginfo(f"V22 F{self.fc} {self.mode} e={error:.0f}")

        except Exception as e:
            import traceback; rospy.logerr(f"V22: {e}\n{traceback.format_exc()}")

if __name__=="__main__":
    rospy.init_node("line_follower_v22")
    LineFollowerV22Mid()
    rospy.spin()
