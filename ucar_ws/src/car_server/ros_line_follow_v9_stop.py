#!/usr/bin/env python3
"""
ROS巡线 V9 — V9 + 停车区白色横线检测
管线: Canny→IPM(dilate15+erode7)→八邻域→中线/墙线→PID

停车逻辑:
  1. 检测IPM ROI底部出现白色横线(横向白像素>60%宽度)
  2. 连续3帧确认后 → 直行进入停车区
  3. 直行STOP_DELAY秒后 → 停车
"""
...
STOP_WIDTH_RATIO = 0.55  # 白像素占比>55%视为停车横线
STOP_CONSECUTIVE = 3     # 连续N帧确认
STOP_DELAY = 2.0         # 确认后直行N秒(实测后调)


class LineFollowerV9Stop:
    def __init__(self):
    ...
    def callback(self, msg):
        try:
            ...
            if not self.is_stopped:
                # 检查ROI底部10行, 是否有横向白线
                bottom_zone = binary[-15:, :]  # 底部15行
                for check_row in range(bottom_zone.shape[0]-1, 0, -1):
                    white_ratio = (bottom_zone[check_row,:]>0).sum() / rw
                    if white_ratio > STOP_WIDTH_RATIO:
            ...
            if self.stop_detected and not self.is_stopped:
                elapsed = (rospy.Time.now() - self.stop_enter_time).to_sec()
            ...
            if Ls and Rs:
                ...
                info=(f"V9 F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V9 F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V9 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V9: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v9")
    LineFollowerV9Stop()
    rospy.spin()
