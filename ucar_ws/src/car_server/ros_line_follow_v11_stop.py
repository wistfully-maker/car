#!/usr/bin/env python3
"""
ROS巡线 V11 — V11 + 停车检测(边界间填充法) + 直行5秒停车
...
class LineFollowerV11Stop:
    def __init__(self):
        ...
        rospy.loginfo("✅ V11 停车检测+直行5秒 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if Ls and Rs:
                ...
                info=(f"V11 F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V11 F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V11 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V11: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v11")
    LineFollowerV11Stop()
    rospy.spin()
