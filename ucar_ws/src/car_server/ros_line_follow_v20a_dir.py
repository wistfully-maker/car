#!/usr/bin/env python3
"""
ROS巡线 V20aa — 中段(方向统计版): 直行2s→巡线→丢线→一拐→右墙→方向统计二拐→停车
策略:
...
class LineFollowerV20aMid:
    ...
    def detect_cross_pattern(self, dirs):
        """方向分散度: 非主方向(3,4,5,6,7)占比>50% → 直角拐弯"""
        if len(dirs) < 20: return False
        recent = dirs[-20:]
        cnt_turn = sum(1 for d in recent if d in [3,4,5,6,7])
        return cnt_turn >= 10
        ...
        try:
            ...
            if Ls and Rs:
                ...
                if self.turn1_done and not self.turn2_done:
                    L_found = self.detect_cross_pattern(Ld)
                    R_found = self.detect_cross_pattern(Rd)
                    if self.fc % 30 == 0:
                        rospy.loginfo(f"🔍 二拐 L={L_found} R={R_found}")
                    if L_found or R_found:
                        self.turn2_consec += 1
                        if self.turn2_consec >= 5:
                ...
                info=f"V20a F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}"
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V20a F{self.fc} LOST",(5,12),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,0,255),1)

            self._dbg(dbg,msg)
            if self.fc%30==0: rospy.loginfo(f"V20a F{self.fc} {self.mode} e={error:.0f}")

        except Exception as e:
            import traceback; rospy.logerr(f"V20a: {e}\n{traceback.format_exc()}")

if __name__=="__main__":
    rospy.init_node("line_follower_v20a")
    LineFollowerV20aMid()
    rospy.spin()
