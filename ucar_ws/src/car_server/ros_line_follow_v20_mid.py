#!/usr/bin/env python3
"""
ROS巡线 V20 — 中段: 直行2s→巡线→丢线直行2s→白线左转45°→强制右墙→停车
策略:
...
class LineFollowerV20Mid:
    ...
    def detect_cross_pattern(self, dirs):
        """方向6计数: 最近30帧≥10个方向6 → 拐角(直道<5%, 拐角>30%)"""
        ...
        try:
            ...
            if Ls and Rs:
                ...
                if self.turn1_done and not self.turn2_done:
                    ...
                    if self.fc % 30 == 0:
                        rospy.loginfo(f"🔍 二拐 L6={Ld.count(6)} R6={Rd.count(6)} L={L_found} R={R_found}")
                ...
                info=f"V20 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}"
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V20 F{self.fc} LOST",(5,12),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,0,255),1)

            self._dbg(dbg,msg)
            if self.fc%30==0: rospy.loginfo(f"V20 F{self.fc} {self.mode} e={error:.0f}")

        except Exception as e:
            import traceback; rospy.logerr(f"V20: {e}\n{traceback.format_exc()}")

if __name__=="__main__":
    rospy.init_node("line_follower_v20_mid")
    LineFollowerV20Mid()
    rospy.spin()
