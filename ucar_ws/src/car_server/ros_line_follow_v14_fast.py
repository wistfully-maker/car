#!/usr/bin/env python3
"""
ROS巡线 V14 — V14 + 停车检测(边界间填充法) + 直行5秒停车
...
# === PID(对标UP主) ===
KP, KI, KD = 0.3, 0.0, 0.1   # 恢复V11的Kp
BASE_SPEED, MAX_ANGULAR = 0.09, 0.5  # 只提到0.09(温和提速)
DEADZONE = 15
LOOKAHEAD = 10  # 恢复V11的10(不盯太远)
...
STOP_DELAY = 8.0         # 直行8秒后停车


class LineFollowerV14Stop:
    def __init__(self):
        ...
        rospy.loginfo("✅ V14 停车检测+直行5秒 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if Ls and Rs:
                ...
                # === 弯曲度检测: 方向反转过多→误判 → 强制左墙 ===
                def count_reversals(dirs):
                    """统计方向突变次数(跳过3次以内的短波动)"""
                    if len(dirs)<20: return 0
                    rev=0
                    for i in range(1,len(dirs)):
                        diff=abs(dirs[i]-dirs[i-1])
                        if diff>3: rev+=1  # 方向突变>135°
                    return rev
                L_rev=count_reversals(Ld)
                R_rev=count_reversals(Rd)
                force_left=(R_rev>8 or L_rev>8)  # 任一边方向乱跳→强制左墙
                ...
                # === 左右墙切换 + 胡萝卜点误差 ===
                if nL_all > 20 and nR_all > 20 and not same_line and not force_left:
                    # 真双线 → 中线巡线
                ...
                info=(f"V14 F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V14 F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V14 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V14: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v14")
    LineFollowerV14Stop()
    rospy.spin()
