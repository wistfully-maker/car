#!/usr/bin/env python3
"""
ROS巡线 V16H — V16H + 十字补线(最小二乘法)
管线: Canny→IPM(dilate15+erode7)→八邻域→十字检测+补线→中线/墙线→PID
"""
...
INIT_TURN_DEG = 55; INIT_TURN_SPEED = 0.96
INIT_TRANSIT_TIME = 1.5; INIT_TRANSIT_SPEED = 0.16
DEADZONE = 15
LOOKAHEAD = 10
...
STOP_CONSECUTIVE = 3     # 连续3帧确认
STOP_DELAY = 8.0
# 十字补线参数
CROSS_SAMPLE_OFFSET = 15   # 从断开处往上采样
CROSS_SAMPLE_END = 5       # 采样终点偏移
CROSS_DIR_SEQ = [4,4,6,6,6]  # 十字方向序列: 4=左,6=下
CROSS_DIR_OFFSETS = [-1,0,3,5,7]  # 检测索引偏移


def slope_calculate(begin, end, border):
    """最小二乘法计算斜率"""
    xsum=ysum=xysum=x2sum=0.0
    for i in range(begin, end):
        xsum+=i; ysum+=border[i]; xysum+=i*border[i]; x2sum+=i*i
    n=end-begin
...
def cross_fill(l_border, r_border, L_pts, R_pts, L_dirs, R_dirs, rh, rw):
    """十字路口检测+最小二乘补线"""
    # 检测十字方向序列
    break_l, break_r = None, None
    for i in range(1, len(L_dirs)-8):
        if (L_dirs[i-1]==4 and L_dirs[i]==4 and
            L_dirs[i+3]==6 and L_dirs[i+5]==6 and L_dirs[i+7]==6):
            break_l = L_pts[min(i, len(L_pts)-1)][1]; break
    for i in range(1, len(R_dirs)-8):
        if (R_dirs[i-1]==4 and R_dirs[i]==4 and
            R_dirs[i+3]==6 and R_dirs[i+5]==6 and R_dirs[i+7]==6):
            break_r = R_pts[min(i, len(R_pts)-1)][1]; break

    if break_l is None or break_r is None: return

    # 补左边界
    start = max(0, break_l - CROSS_SAMPLE_OFFSET)
    end = max(0, break_l - CROSS_SAMPLE_END)
    k = slope_calculate(start, end, l_border)
    x_avg = (start+end)//2
    y_avg = np.mean(l_border[start:end]) if end>start else l_border[start]
    b = y_avg - k*x_avg
    for y in range(end, rh):
        l_border[y] = min(max(int(k*y+b), 0), rw-1)

    # 补右边界
    start = max(0, break_r - CROSS_SAMPLE_OFFSET)
    end = max(0, break_r - CROSS_SAMPLE_END)
    k = slope_calculate(start, end, r_border)
    x_avg = (start+end)//2
    y_avg = np.mean(r_border[start:end]) if end>start else r_border[start]
    b = y_avg - k*x_avg
    for y in range(end, rh):
        r_border[y] = min(max(int(k*y+b), 0), rw-1)


class LineFollowerV16Hybrid:
    def __init__(self):
        ...
        self.fc = 0
        self.mode = "INIT"
        # 前置阶段: 0=直行,1=转,2=过渡,3=巡线
        self.phase = 0; self.phase_time = None
        # 停车
        ...
        rospy.loginfo("✅ V16H 停车检测+直行5秒 启动")
    ...
    def callback(self, msg):
        try:
            ...
            if self.phase < 3:
                ...
                if self.phase == 0:
                    if el < INIT_STRAIGHT_TIME:
                        ...
                        rospy.loginfo(f"⏸ 直行完成→左转{INIT_TURN_DEG}°")
                elif self.phase == 1:
                    td=math.radians(INIT_TURN_DEG)/INIT_TURN_SPEED
                    if el < td:
                        t=Twist(); t.angular.z=INIT_TURN_SPEED; self.pub_cmd.publish(t); return
                    else:
                        self.phase=2; self.phase_time=rospy.Time.now()
                        rospy.loginfo("↩ 旋转完成→低速直行")
                elif self.phase == 2:
                    if el < INIT_TRANSIT_TIME:
                        t=Twist(); t.linear.x=INIT_TRANSIT_SPEED; self.pub_cmd.publish(t); return
                    else:
                        self.phase=3; self.phase_time=None; self.last_error=0.0; self.integral=0.0
                        rospy.loginfo("✅ 前置完成, 开启V16H视觉巡线!")
                return
            ...
            if Ls and Rs:
                ...
                # === 十字补线(检测方向序列+最小二乘补边界) ===
                cross_fill(lb, rb, Lp, Rp, Ld, Rd, rh, rw)
                ...
                info=(f"V16H F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V16H F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"V16H F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"V16H: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v16_hybrid")
    LineFollowerV16Hybrid()
    rospy.spin()
