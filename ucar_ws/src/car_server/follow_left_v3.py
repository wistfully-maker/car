...
class FollowLeftV3:
    def __init__(self):
        rospy.init_node('follow_left_v3', anonymous=True)
        ...
        # 阶段2: 接近拐角
        self.bot_white_need = 50; self.approach_wait = 0
        ...
        self.fc = 0
        rospy.loginfo("✅ follow_left_v3 | 检测方向0→接近拐角→旋转70°")
...
# ===== 阶段3: 旋转完成 → 直接摆正直行 =====
            if self.rotate_flag == 1:
                ...
                self.rotate_flag = 2
                rospy.loginfo("✅ 旋转70°完成! → 摆正直行2s..."); return
            ...
            if Ls and Rs:
                Lp, Ld = self.trace(mask, Ls, SEEDS_R)
                Rp, Rd = self.trace(mask, Rs, SEEDS_L)
                ...
                if not self.corner_seen:
                    if l0 > self.dir0_thr and r0 > self.dir0_thr:
                        ...
                        if self.dir0_consec >= self.dir0_consec_need:
                            self.corner_seen = True; self.approach_wait = 10
                ...
                if self.corner_seen and not self.rotate_flag:
                    if self.approach_wait > 0: self.approach_wait -= 1
                    elif track_width >= self.bot_white_need:
                        rospy.loginfo(f"🎯 拐角够近! 赛道宽={track_width} → 旋转{self.rotate_deg}°")
                ...
                if self.fc % 15 == 0:
                    ...
                    if self.corner_seen: info += f" 冷却={self.approach_wait}"
                    rospy.loginfo(info)
