...
# -*- coding: utf-8 -*-
"""
follow_right_v7 — 基于 V7（right 镜像版），停车机制「识别到前停车线后 0.3m/s 匀速 1.9s → 停车」
拐角确认(L0%>40% AND R0%>40%连续3帧) → 低速前进APPROACH_TIME秒 → 旋转-65° → 微调 → 摆正直行2s → 巡航8s
→ 检测前停车线 → 转14°摆正 → 0.3m/s 匀速 1.9s → 停车

相对 left_v7 的改动（左右镜像）：
  1. 旋转方向：65° → -65°
  2. 前线微调：rotate(-16) → rotate(14)
  其余停车逻辑与 left_v7 完全一致

用法: rosrun car_server follow_right_v7.py
"""
...
class FollowRightV7:
    def __init__(self):
        rospy.init_node('follow_right_v7', anonymous=True)
        ...
        self.fc = 0
        rospy.loginfo(f"✅ follow_right_v7 前停车线后 {self.stop_speed}m/s 匀速 {self.stop_dur}s → 停车")
