#!/usr/bin/env python3
"""
ROS巡线 左道完整版 = pre_launch_left + LF
步骤: 直行1.8s(0.2m/s)→逆时针转70°→直行1s→视觉巡线→白线直行4.7s停车
"""
...
BASE_SPEED, MAX_ANGULAR = 0.18, 0.28
STOP_SPEED = 0.3
DEADZONE = 15; LOOKAHEAD = 10
# 前置动作(pre_left参数)
INIT_STRAIGHT_TIME = 1.8; INIT_SPEED = 0.2
INIT_TURN_DEG = 70; INIT_TURN_SPEED = 1.0
INIT_STRAIGHT2 = 1.0  # 转完直行1s
# 停车检测
STOP_WIN_H = 10; STOP_WIN_W = 60; STOP_WHITE_THRESH = 0.40  # 加宽窗口, 降阈值
STOP_CONSECUTIVE = 1; STOP_DELAY = 2.1


class LineFollowerV21Left:
    def __init__(self):
        ...
        try:
            ...
            if self.phase < 3:
                ...
                if self.phase == 0:
                    ...
                    else: self.phase=1; self.phase_time=rospy.Time.now()
                elif self.phase == 1:
                    td=math.radians(INIT_TURN_DEG)/INIT_TURN_SPEED
                    if el < td:
                        t=Twist(); t.angular.z=INIT_TURN_SPEED; self.pub_cmd.publish(t)
                    else: self.phase=2; self.phase_time=rospy.Time.now()
                elif self.phase == 2:
                    if el < INIT_STRAIGHT2:
                        t=Twist(); t.linear.x=INIT_SPEED; self.pub_cmd.publish(t)
                    else: self.phase=3; self.phase_time=None; self.last_error=0.0; self.integral=0.0; rospy.loginfo("✅ 前置完成, phase=3 开始巡线!")
                return
            ...
            if Ls and Rs:
                ...
                info=(f"LF F{self.fc} {self.mode} e={error:.0f} "
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"LF F{self.fc} LOST",(5,12),
            ...
            if self.fc%30==0:
                rospy.loginfo(f"LF F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}")

        except Exception as e:
            import traceback; rospy.logerr(f"LF: {e}\n{traceback.format_exc()}")


if __name__=="__main__":
    rospy.init_node("line_follower_v21_left")
    LineFollowerV21Left()
    rospy.spin()
