...
# -*- coding: utf-8 -*-
"""
follow_left_v7 — 基于 V5，停车机制改为「识别到前停车线后 0.3m/s 匀速 3s → 停车」
拐角确认(L0%>40% AND R0%>40%连续3帧) → 低速前进APPROACH_TIME秒 → 旋转65° → 微调 → 摆正直行2s → 巡航8s
→ 检测前停车线 → 转-16°摆正 → 0.3m/s 匀速 3s → 停车

相对 V5 的改动：
  1. 去掉「固定37帧后线检测」，改为识别到前停车线后计时：0.3m/s 行驶 3s 直接停车
  2. 停车位置由「时间×速度」决定，不依赖后线在画面里的位置
  3. 保留：全黑强制停车（撞墙兜底）

用法: rosrun car_server follow_left_v7.py
"""
...
class FollowLeftV7:
    def __init__(self):
        rospy.init_node('follow_left_v7', anonymous=True)
        ...
        self.fc = 0
        rospy.loginfo(f"✅ follow_left_v7 前停车线后 {self.stop_speed}m/s 匀速 {self.stop_dur}s → 停车")
