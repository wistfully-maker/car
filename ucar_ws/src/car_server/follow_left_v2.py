...
用法: rosrun car_server follow_left_v2.py
"""
...
class FollowLeftV2:
    def __init__(self):
        rospy.init_node('follow_left_v2', anonymous=True)
        ...
        self.rotate_flag = 0; self.corner_seen = False
        ...
        self.fc = 0
        rospy.loginfo("=" * 50)
        rospy.loginfo("✅ follow_left_v2 左道 [八邻域方向0检测+逆时针70°]")
                ...
                if not self.rotate_flag:
                    if (l0_pct > self.dir0_thr or r0_pct > self.dir0_thr) and (srow is not None and srow > 90):
                        ...
                        if self.dir0_consec >= self.dir0_consec_need:
                            rospy.loginfo(f"🔀 拐角特征! L0%={l0_pct*100:.0f} R0%={r0_pct*100:.0f} → 继续接近..."); self.corner_seen = True
