#!/usr/bin/env python3
"""
ROS巡线 V19 — 中段: 直行2s→巡线→丢线直行2s→白线左转45°→强制右墙→停车
策略:
...
LOST_STRAIGHT = 2.0; LOST_SPEED = 0.15   # 丢线直行2s
TURN_ANGLE = 45; TURN_SPEED = 0.785
STOP_THRESH = 0.40; STOP_CONS = 1; STOP_DELAY = 4.5


class LineFollowerV19Mid:
    def __init__(self):
    ...
    def callback(self, msg):
        try:
            ...
            # === 丢线处理: 直行2s接近转向线 ===
            if not self.turn1_done and Ls is None and Rs is None:
                if self.lost_start is None:
                    self.lost_start = rospy.Time.now()
                    rospy.logwarn("⚠️ 丢线! 直行2s...")
                ...
                if el < LOST_STRAIGHT:
                    ...
                    self._dbg(dbg,msg); return
                else: self.lost_start=None
            elif Ls and Rs:
                self.lost_start = None

            # === 左转线检测(顶部+底部双窗口) ===
            if not self.turn1_done and not self.is_stopped:
                top = binary[0:50, rw//4:3*rw//4]; tw=(top>0).sum()/top.size if top.size>0 else 0
                bot = binary[rh-40:rh, rw//4:3*rw//4]; bw=(bot>0).sum(axis=1)/bot.shape[1]
                wr = max(tw, bw.max())
                if wr >= STOP_THRESH:
                    self.turn1_consec += 1
                    rospy.loginfo(f"↪ 左转线{wr*100:.0f}% [{self.turn1_consec}/{STOP_CONS}]")
                else: self.turn1_consec = max(0,self.turn1_consec-1)
                if self.turn1_consec >= STOP_CONS:
                    td=math.radians(TURN_ANGLE)/TURN_SPEED
                    rospy.loginfo(f"↪ 左转{TURN_ANGLE}°"); t0=rospy.Time.now()
            ...
            if Ls and Rs:
                ...
                spd = 0.08 if self.turn1_done else BASE_SPEED
                ...
                info=f"V19 F{self.fc} {self.mode} e={error:.0f} hw={self.road_half}"
                ...
                self.mode="LOST"
                dbg=cv2.cvtColor(binary,cv2.COLOR_GRAY2BGR)
                cv2.putText(dbg,f"V19 F{self.fc} LOST",(5,12),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,0,255),1)

            self._dbg(dbg,msg)
            if self.fc%30==0: rospy.loginfo(f"V19 F{self.fc} {self.mode} e={error:.0f}")
