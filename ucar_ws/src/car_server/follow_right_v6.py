...
# -*- coding: utf-8 -*-
"""
follow_right_v6 — 时间制接近拐角（右转版，同步 left_v5 的停车保护）
拐角确认(L0%>40% AND R0%>40%连续3帧) → 低速前进APPROACH_TIME秒 → 旋转-65° → 微调 → 摆正直行2s → 停车

停车保护（同 left_v5）：
  1. 正常后线停车：37帧后检测到白线
  2. 全黑强制停车：白色像素<30连续5帧
  3. 超时强制停车：前线后2.4秒

用法: rosrun car_server follow_right_v6.py
"""
...
class FollowRightV6:
    def __init__(self):
        rospy.init_node('follow_right_v6', anonymous=True)
        ...
        self.cruise_start = None; self.cruise_dur = 7.5; self.cruise_done = False

        # 停车
        self.stop_flag = False
        self.stop_front_found = False; self.stop_fps = 0
        self.stop_fps_thr = 34; self.stop_voice = False
        self.stop_rs = 24; self.stop_re = 119
        self.stop_f_thr = 60; self.stop_b_thr = 60

        # 全黑强制停车（撞墙保护）
        self.black_consec = 0             # 全黑连续帧数
        self.black_pixel_thr = 30         # 白色像素 < 30 判定全黑
        self.black_consec_need = 5        # 连续 5 帧全黑强制停车

        # 超时强制停车（识别到前线后超时保护）
        self.stop_front_time = None       # 识别到前线的时间戳
        self.stop_timeout = 1.8           # 前线后行驶超此时长强制停车（秒）

        self.fc = 0
        rospy.loginfo(f"✅ follow_right_v6 时间制接近={APPROACH_TIME}s → 旋转-65°")
    ...
    def detect_stop_line(self, bgr):
        # 超时强制停车：识别到前线后行驶超时 → 强制停车（兜底）
        if self.stop_front_found and self.stop_front_time is not None:
            if (rospy.Time.now() - self.stop_front_time).to_sec() >= self.stop_timeout:
                rospy.logwarn("⏱ 识别到前线后超时，强制停车！")
                return True
        ...
        if len(rf)>=6:
            ...
            if sum(rf)/len(rf)>thr:
                if not self.stop_front_found:
                    ...
                    self.stop_front_time = rospy.Time.now()  # 记录前线时间戳
                    return False
            ...
            if self.post_done:
                if not self.cruise_done:
                    ...
                    if el_c < self.cruise_dur:
                        ...
                        if self.fc%30==0: rospy.loginfo(f"🚫 屏蔽停车 {el_c:.1f}s/{self.cruise_dur}s")
                        return
                    self.cruise_done = True
                    rospy.loginfo("🔊 巡航结束！开始检测停车线...")
                # 全黑强制停车（撞墙保护）：画面几乎无白线 → 连续5帧 → 停车
                if np.count_nonzero(mask) < self.black_pixel_thr:
                    self.black_consec += 1
                    if self.black_consec >= self.black_consec_need:
                        rospy.logwarn(f"⚠️ 连续{self.black_consec}帧全黑，强制停车！")
