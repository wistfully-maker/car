...
# -*- coding: utf-8 -*-
"""
follow_right — 右道专用, 100%提取自follow.py + 顺时针70°旋转
...
用法: rosrun car_server follow_right.py
"""
...
class FollowRight:
    def __init__(self):
        rospy.init_node('follow_right', anonymous=True)
        ...
        rospy.loginfo("=" * 50)
        rospy.loginfo("✅ follow_right 右道启动 [顺时针70°]")
        rospy.loginfo("   任务1: 原地对正")
        rospy.loginfo("   任务2: 直行0.3s")
        rospy.loginfo("   任务3: 凸包检测拐角 → 原地顺时针70°")
        ...
        if len(rows_found) >= 6:
            ...
            if sum(rows_found) / len(rows_found) > stop_threshold:
                if self.is_stop_line_front_found == False:
                    print("找到终点停车线的前线")
                    self.rotate_speed(-20, 0.3)  # 右道用负20°
                    rospy.sleep(0.3)
                    ...
                    if tracker_result is not None:
                        ...
                        if (left_count > self.threshold or right_count > self.threshold) or self.rotate_one_flag == 1:
                            if self.rotate_one_flag == 0:
                                rospy.loginfo(f"🔄 [任务3/4] 检测到拐点! 顺时针旋转{abs(self.rotate_val)}°")
