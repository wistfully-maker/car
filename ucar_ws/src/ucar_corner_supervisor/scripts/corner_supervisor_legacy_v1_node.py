...
"""Frozen ROS node for the first-version corner supervisor (19e0ce7)."""
...
from ucar_corner_supervisor.legacy_v1_geometry import (
    Supervisor,
    SupervisorConfig,
    find_first_corner,
)
...
class LegacyV1CornerSupervisorNode:
    def __init__(self):
        ...
        self._diagnostic_publisher = rospy.Publisher(
            "~diagnostic", String, queue_size=10
        )
        rospy.Subscriber(
            RAW_COMMAND_TOPIC,
            Twist,
    ...
    def _assert_cmd_vel_is_unowned(cls):
        ...
        if owners:
            raise RuntimeError(
                "{} already has publishers: {}. Stop every navigation "
                "and velocity controller before switching versions.".format(
    ...
    def _control_callback(self, _event):
        ...
        if (
            ...
            try:
                ...
                if other_publishers:
                    message = (
                        "conflicting /cmd_vel publishers: {}".format(
                ...
                rospy.logwarn_throttle(
                    2.0,
                    "cannot verify /cmd_vel ownership: %s",
                    error,
                )
        ...
        try:
            ...
            rospy.logwarn_throttle(
                2.0,
                "legacy v1 corner supervisor TF: %s",
                error,
            )
        ...
        if result.state != self._last_state:
            rospy.loginfo(
                "legacy v1 corner supervisor state: %s",
                result.state,
            )
...
def main():
    rospy.init_node("ucar_corner_supervisor_legacy_v1")
    try:
        LegacyV1CornerSupervisorNode()
    except Exception as error:
        rospy.logfatal(
            "legacy v1 corner supervisor failed to start: %s",
            error,
        )
        raise
    rospy.spin()
