"""Frozen first-version geometry and velocity state machine.

This module intentionally mirrors commit 19e0ce7 so the proven first
implementation can be selected without changing the current implementation.
"""
...
class Supervisor:
    ...
    def _turn_command(self, yaw):
        ...
        if abs(error) <= self.config.heading_tolerance:
            return 0.0
        speed = min(
            self.config.turn_max_angular,
            self.config.turn_kp * abs(error),
        )
    ...
    def update(
        ...
        if self.state == "ERROR":
            return self._stop(
                "supervisor error is latched until the goal clears"
            )
        if not tf_valid:
            self.state = "ERROR"
        ...
        if self.state in ("TURNING", "EXIT_ALIGN"):
            if not raw_fresh:
                self.state = "ERROR"
                return self._stop(
                    "raw velocity command became stale during turn"
                )
            if now - self.turn_started_at > self.config.turn_timeout:
                self.state = "ERROR"
                return self._stop("turn timeout")
            heading_error = abs(
                normalize_angle(self.target_heading - yaw)
            )
            if heading_error <= self.config.heading_tolerance:
                ...
                if (
                    now - self.aligned_since
                    >= self.config.heading_hold_time
                ):
                    self.state = "FOLLOWING"
                    self.target_heading = None
                    self.turn_started_at = None
...
def resample_polyline(points, spacing):
    ...
    if math.hypot(
        source[-1][0] - sampled[-1][0],
        source[-1][1] - sampled[-1][1],
    ) > 1.0e-9:
