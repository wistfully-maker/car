...
中间航点不等待 `move_base` 报告完整到达，也不使用“到达后发送下一目标”的串行
方式。满足以下全部条件时提前发送下一个目标：
...
TEB在中间航点阶段启用 `free_goal_vel: true`。切换半径必须大于当前速度对应的制动
距离、一次全局重规划延迟和控制延迟之和，使下一个目标在TEB开始为当前中间目标
减速前生效。发送下一个action目标时不得先显式取消旧目标，也不得插入零速度。

中间航点的实车验收不是“没有长时间停车”，而是穿越切换区域期间里程计前进速度
始终高于配置的穿越速度下限。若新旧目标切换导致底盘实际停止，该轮不通过，必须
调整切换半径或目标衔接，不接受停车后重新启动。
